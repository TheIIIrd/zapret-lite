...
# Интервал автоперезапуска: off либо целые часы от 6 до 24.
#
# Диапазон узкий намеренно. Чаще шести часов - это уже не подстраховка,
# а маскировка проблемы: обход будет казаться работающим, а причина
# останется ненайденной. Реже суток бессмысленно.
zl_valid_interval() {
	case "$1" in
		off) return 0 ;;
		[6-9]h|1[0-9]h|2[0-4]h) return 0 ;;
		*) return 1 ;;
	esac
}
